...
def process_id_chain_list(id_chain_list):
    df = pd.read_csv("PDB_benchmark_structures\\lengths.csv", delimiter=',', header=0)
    return df
...
def split_headerandseq(msa_file, len_a, limit=-1):
    ...
    for idx, line in enumerate(lines):
        ...
        if idx == 0:
            line = '>BNULL'+'_'+'N1'+'_'+'N2'
        if line[0] == '>':
            if len(header_a) == -1:
                break
            header_a.append(line[1:].split('_')[1])
            header_b.append(line[1:].split('_')[2])
        else:
...
### PARSE NUMBER OF CORES FROM SLURM ###
    ...
    msa_list = glob.glob(msadir + '*.fas')
    msa_files = msa_list
    df_id_list = process_id_chain_list(msadir+'id_chains.csv')
...
### MAIN LOOP ###
    for msa_in in msa_files:
        msa_ext = '.fas'
        msa_proc = msa_in.split('/')[-1]
        msa_name = msa_proc.split(msa_ext)[0]
        pdbid = msa_name.split('_')[0]
        prot_len = int(df_id_list[df_id_list['PDBid']==pdbid].iloc[0]['Length'])
